...
build_dev:
	docker build --no-cache -t civilcode/elixir-dev:$(VERSION)$(SUFFIX) -f elixir/Dockerfile.dev .
...
build_ci:
	docker build --no-cache -t civilcode/elixir-ci:$(VERSION)$(SUFFIX) -f elixir/Dockerfile.ci .
